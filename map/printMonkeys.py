...
def printMonkeys(color, index, quest):
    ...
    print(color.setBackground('darkGreen', position((config.questToDo[index][quest][0])*2+3, (config.questToDo[index][quest][1])+1, emojiDecoder('f09f90b5'))))
    print(color.setBackground('darkGreen', position((config.questToDo[index][quest][0])*2+1, (config.questToDo[index][quest][1]), emojiDecoder('f09f90b5'))))
